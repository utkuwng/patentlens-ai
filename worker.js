...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (!new Set(["/", "/index.html", "/app.js", "/style.css"]).has(url.pathname)) {
      ...
      // Prevent redirect loop: with default HTML canonicalization, /index.html
      // may redirect to /, while rewriting / to /index.html here redirects back.
      // Request the original path first and let Cloudflare serve index.html at /.
      // If assets return 404 for / with html_handling=none, fetch the HTML file
      // internally, keeping the browser URL unchanged. Never forward a redirect
      // from /index.html back to /; that would reintroduce the loop.
      if (url.pathname === "/" || url.pathname === "/index.html") {
        const rootURL = new URL(request.url);
        rootURL.pathname = "/";
        let page = await env.ASSETS.fetch(new Request(rootURL, request));
        if (page.status === 404) {
          const fileURL = new URL(request.url);
          fileURL.pathname = "/index.html";
          page = await env.ASSETS.fetch(new Request(fileURL, request));
        }
        if (page.status >= 300 && page.status < 400) {
          return new Response("Trang chu bi loi dieu huong trong Static Assets. Kiem tra html_handling=none va deploy lai toan bo repo.", {
            status: 502,
