/* PatentLens v40 — Five-stage research prototype. Frontend does not carry API keys. */
'use strict';
...
const S={stage:0,mode:'prospective',source:'',filename:'',fileMeta:null,pdf:null,pages:[],unread:[],pdfBusy:false,pdfAbort:false,uploadId:0,ocrToken:0,claims:[],keywordOnly:true,route:null,routeAudit:null,concepts:[],claimDates:{},queries:[],queryLog:[],candidates:[],prior:{D1:null,D2:null,D3:null},features:[],matrix:{},ai:{},aiCoverage:{},searching:false,searchAbort:null,aiBusy:false,aiStop:false,expertNote:'',signed:false,reportHtml:'',revision:0,expectedClaims:0,autoReadBusy:false};
...
function invalidate(from){S.autoComputed=false;S.revision++;if(from<=1){S.route=null;S.concepts=[];S.queries=[];}if(from<=2){S.queryLog=[];S.candidates=[];S.prior={D1:null,D2:null,D3:null};}if(from<=3){S.features=[];S.matrix={};S.ai={};S.aiCoverage={};S.claimDates={};if($('claimInputVerified'))$('claimInputVerified').checked=false;}S.reportHtml='';}
...
function renderMatrix(){
 ...
 root.innerHTML=S.features.map((f,i)=>`<article class="matrix-row" data-feature="${i}"><header><div><span class="tag">${esc(f.id)}</span>${f.inheritedFrom?` <span class="tag yellow">Kế thừa claim ${f.inheritedFrom}</span>`:''}<p>${esc(f.text)}</p></div></header><div class="matrix-cols">${['D1','D2','D3'].map(slot=>{
...
function assessmentHtml(){
 ...
 if(S.expectedClaims&&S.claims.length<S.expectedClaims)gaps.push(`chỉ phát hiện ${S.claims.length}/${S.expectedClaims} claims ghi trên bìa`);
...
function reportCreate(){
 ...
 const statusRows=S.features.flatMap(f=>slots.map(slot=>{const d=S.prior[slot],a=S.ai[matrixKey(f.id,slot)]||{},r=S.matrix[matrixKey(f.id,slot)]||{};return `<tr><td>${esc(f.id)}</td><td>${esc(f.text)}</td><td>${slot} · ${esc(d.no)}</td><td>${esc(a.status||'Chưa có dữ liệu')} ${esc(a.engine||'')}</td><td>${esc(r.checked?r.status:'Chưa rà soát')}</td><td>${a.evidence&&quotedInDocument(a.evidence,d.text)?highlightLiteral(a.evidence,a.match||a.evidence):'Chưa có câu trích khớp chữ'}</td></tr>`;})).join('');
 S.reportHtml=`<h2>PatentLens AI · Báo cáo ${S.signed?'(đã ghi nhận chuyên gia xem)':'(DỰ THẢO MÁY — CHƯA DUYỆT)'}</h2><p>Đầu vào: ${esc(S.filename||'văn bản được cung cấp')} · ${S.keywordOnly?'Chế độ keyword-only':'Claims: '+S.claims.length} · Ngày tạo: ${esc(new Date().toLocaleString('vi-VN'))}</p><h3>Nhận định ban đầu</h3>${assessmentHtml()}<h3>Ba nguồn ưu tiên</h3><table><thead><tr><th>Thứ tự</th><th>Mã công bố</th><th>Ngày công bố</th><th>Độ phủ nội dung lấy được</th><th>Nguồn</th></tr></thead><tbody>${sourceRows}</tbody></table><h3>D1 → D3: đoạn trùng được đánh dấu</h3>${evidenceByDocumentHtml()}<h3>Bảng phân biệt máy / chuyên gia</h3><table><thead><tr><th>ID</th><th>Input</th><th>Tài liệu</th><th>Máy gợi ý</th><th>Chuyên gia</th><th>Đoạn nguồn</th></tr></thead><tbody>${statusRows}</tbody></table><h3>Nhận xét chuyên gia (tùy chọn)</h3><p>${esc(S.expertNote||'Chưa có.')}</p><p>${S.signed?'Người dùng đã đánh dấu đã xem; không có xác thực danh tính/chữ ký số.':'Chưa có xác nhận chuyên gia.'}</p><p><strong>Phạm vi:</strong> Các trang OCR lỗi, bản vẽ, trích xuất thiếu toàn văn, ngày ưu tiên và mức bao phủ tìm kiếm phải kiểm tra bằng bản gốc. Trùng thuật ngữ không đồng nghĩa bộc lộ đầy đủ một claim. Kết quả không thay thế ý kiến pháp lý hoặc đánh giá trình độ sáng tạo của chuyên gia.</p>`;
