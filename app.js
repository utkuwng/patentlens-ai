...
async function readPdf(file){
  ...
  for(let p=1;p<=pdf.numPages;p++){
    ...
    $("progressBar").style.width=Math.round(p/pdf.numPages*78)+"%";
    $("pdfStatus").textContent=`Đang đọc lớp chữ PDF: trang ${p}/${pdf.numPages} · chưa chạy OCR`; 
    if(p%3===0 || p===pdf.numPages)await sleep(0); // nhường luồng để Chrome cập nhật tiến độ
...
function makeInputScanAudit(){
  ...
  state.inputScanAudit={pages:total,ocrNeeded:pending.length+Object.values(state.ocrPages).filter(x=>String(x||'').trim().length>=25).length,
    ...
    pending,unread:(existing.unread||[]).filter(x=>pending.includes(x)),
    visualsReviewed:false,checkedAt:new Date().toISOString(),
    complete:pending.length===0&&!existing.cancelled,cancelled:false};
...
function renderPdfAuditStatus(){
  ...
  if(b){b.hidden=left===0;b.disabled=!!state.inputAuditBusy||!!state.claimOcrBusy;b.textContent=left?`Đọc bổ sung ${left} trang cần OCR`: 'Đã xử lý xong chữ';}
  if(stop)stop.hidden=!state.inputAuditBusy;
  if(status)status.textContent=left
    ?`PDF có ${audit.pages} trang. Đã quét lớp chữ tất cả trang; ${left} trang còn thiếu/khó đọc, chưa xác minh toàn bộ. Có thể tiếp tục xem claims; trước khi nhận định cần xử lý hoặc kiểm tra các trang còn thiếu.`
...
async function getOcrWorker(reason='OCR',lang='vie'){
  if(!window.Tesseract)throw new Error('Tesseract.js chưa tải được; có thể nhập/sửa claims ở Bước 2.');
...
async function processFile(file){
  ...
  state.pdfLoading=true;state.auditStop=true;state.inputAuditBusy=false;
  ...
  for(const slot of ["D1","D2","D3"]){const n=slot.slice(1);for(const k of ["No","Date","Url","Text"]){const el=$(`d${n}${k}`);if(el)el.value="";}}
  state.ocrPages={};state.inputScanAudit=null;
  state.claims=[];
  state.claimsText="";
  state.features=[];
  state.search=[];
  ...
  if(state.claims.length){
    ...
    $("pdfStatus").textContent=`Đã nhận diện ${state.claims.length} claim từ lớp chữ/OCR đã chạy · PDF ${state.pdf.numPages} trang. ${pending.length?'⚠ Còn '+pending.length+' trang chưa đọc đủ chữ. Chuyển sang Bước 2 để xem claims; có thể chạy OCR bổ sung tại đó.':'Đã thử đọc chữ mọi trang; vẫn cần xem hình vẽ trong bản gốc.'}`;
  }else{
    setDetect("detClaims",false,"Chưa tự tách được claim");
    $("pdfStatus").textContent=`Đã quét lớp chữ ${state.pdf.numPages} trang và OCR thử trang cuối khi cần nhưng chưa tìm rõ claims. Chuyển Bước 2 để chạy OCR bổ sung hoặc sửa claims; KHÔNG xem như đã đọc đủ.`;
  }
  state.pdfLoading=false;
  renderPdfAuditStatus();
  if(state.step===0&&['patentability','pilot'].includes($("purposeChoice")?.value||''))showStep(1);
  if(queuedLongClaims){
    $('pdfStatus').textContent=`Đã quét lớp chữ ${state.pdf.numPages} trang. Đang thử OCR tìm claims trên 2 trang cuối ở chế độ nền; bạn có thể xem Bước 2 ngay.`;
    const currentPdf=state.pdf;
    setTimeout(async()=>{
      if(state.pdf!==currentPdf||state.uploadGeneration!==generation||state.claims.length)return;
      try{
        const ok=await smartOcrClaims(true);
        if(state.pdf!==currentPdf||state.uploadGeneration!==generation)return;
        makeInputScanAudit();
        if(!ok&&!state.claims.length)$('pdfStatus').textContent='OCR nhanh chưa đọc được claims. Bạn có thể quét thêm trang hoặc nhập/sửa claims ở Bước 2; đây chưa phải đã đọc đủ tài liệu.';
        renderPdfAuditStatus();renderInputGate();
      }catch(err){if(state.pdf===currentPdf)$('pdfStatus').textContent='OCR nhanh gặp lỗi: '+String(err?.message||err)+'. Bạn có thể dùng Bước 2 hoặc OCR bổ sung.';}
    },0);
  }
}
