...
function showStep(i){
  ...
  if(STEPS[state.step].id==="search"){
    if(state.features.length&&!state.queries.length){state.search=buildProSearchRows();state.queries=buildProQueries(state.search);renderSearch();}
...
// Every PDF page is inspected. Unreadable pages are retried with OCR; OCR failures stay visible.
async function auditAllInputPages(){
  if(!state.pdf)return;
  const total=state.pdf.numPages,unread=[],pages=[];
  for(let p=1;p<=total;p++){
    const direct=String(state.pageText[p-1]||'');
    if((state.pageQuality[p-1]||0)<70||direct.trim().length<25)pages.push(p);
  }
  state.inputScanAudit={pages:total,ocrNeeded:pages.length,ocrCompleted:0,unread:[],visualsReviewed:false,checkedAt:new Date().toISOString()};
  for(const p of pages){
    const ok=await ocrSelectedPages([p],'OCR toàn tài liệu: '+p+'/'+total);
    const content=String(state.ocrPages[p]||state.pageText[p-1]||'');
    if(!ok||content.trim().length<25||((state.pageQuality[p-1]||0)<48&&!state.ocrPages[p]))unread.push(p);else state.inputScanAudit.ocrCompleted++;
    state.inputScanAudit.unread=[...unread];
    if($('pdfStatus'))$('pdfStatus').textContent=`Kiểm tra trang PDF ${p}/${total}; chưa đọc được: ${unread.length}.`;
    await sleep(0);
  }
  const audit=state.inputScanAudit;
  if($('pdfStatus'))$('pdfStatus').textContent=`Đã quét ${total} trang; ${pages.length} trang cần OCR, ${audit.ocrCompleted} trang OCR có văn bản, ${unread.length} trang còn thiếu. Hình vẽ chưa được AI diễn giải.`;
  state.reviewLog.push({time:new Date().toISOString(),action:'pdf_page_coverage',pages:total,ocrNeeded:pages.length,unread:[...unread]});
}
function pdfCoverageReady(){return !state.pdf||!!(state.inputScanAudit&&state.inputScanAudit.pages===state.pdf.numPages&&!state.inputScanAudit.unread?.length)}
...
async function processFile(file){
  ...
  // Page coverage is audited before we claim the PDF has been scanned; never silently skip middle pages.
  try{await auditAllInputPages()}catch(e){
    state.inputScanAudit=state.inputScanAudit||{pages:state.pdf?.numPages||0,unread:[]};
    state.inputScanAudit.unread.push('OCR error: '+String(e.message||e));
    console.warn('Full-page audit incomplete',e);
  }
  ...
  if(state.claims.length){
    ...
    $("pdfStatus").textContent=`Hoàn tất bước đọc chữ. ${mode} ${missed.length?'⚠ Còn trang chưa đọc được: '+missed.join(', ')+'. Không được dùng kết quả như đã đọc hết PDF.':'Đã thử kiểm tra mọi trang chữ, hình vẽ chưa được AI diễn giải.'}`;
  }else{
    setDetect("detClaims",false,"Chưa tự tách được claim");
    $("pdfStatus").textContent="Đã xử lý PDF. Máy chưa tự tách được claims; hãy xem và sửa nội dung ở bước 2.";
  }
  state.pdfLoading=false;
...
function renderCandidates(){
  ...
  $("candidateBody").innerHTML=state.candidates.map((c,i)=>{
    ...
    return `<tr class="${selected.length?"candidate-selected-row":""}">
      ...
      <td>${esc(date)}</td>
      <td><span class="score ${scoreCls}">Ưu tiên đọc: ${c.score>=65?"Cao":c.score>=35?"Vừa":"Thấp"}</span></td>
...
async function searchRealPatents(){
  if(!(state.features||[]).length){alert("Chưa tách được đặc điểm từ các claims. Hãy kiểm tra PDF ở Bước 2.");return;}
  const g=inputGate(false);if(!g.ok){renderInputGate();alert("Cần chọn mục đích và có phần claims trước khi tra cứu.\n"+g.issues.join("\n"));return}
